    ...
    <div style={{ position: "fixed", inset: 0, zIndex: 0, pointerEvents: "none" }}>
      {particles.map(p => (
        <div key={p.id} style={{
          ...
          borderRadius: "50%", background: "#ffb800",
          opacity: p.opacity,
    ...
    <div style={{ position: "relative", width: "60px", height: "60px", margin: "0 auto 20px" }}>
      {[0, 1, 2].map(i => (
        <div key={i} style={{
          ...
          borderRadius: "50%",
          opacity: 0,
          animation: `radarPulse 2s ${i * 0.6}s infinite ease-out`,
        }} />
      ))}
      <div style={{
  ...
  return (
    <div style={{ minHeight: "100vh", background: "#070b10", fontFamily: "'IBM Plex Mono','Courier New',monospace", color: "#c8d4e0", overflowX: "hidden" }}>
      ...
      <div style={{
        ...
        background: "radial-gradient(ellipse at center, rgba(255,184,0,0.07) 0%, rgba(255,184,0,0.02) 40%, transparent 70%)",
      ...
      <div style={{
        ...
        <div style={{ textAlign: "center", marginBottom: "48px" }}>
          ...
          <div style={{
            ...
            color: "#ffb800", padding: "6px 18px", borderRadius: "4px",
            fontSize: "10px", letterSpacing: "0.15em", marginBottom: "28px",
            textTransform: "uppercase", animation: "borderGlow 3s infinite",
          }}>
          ...
          {/* Title with glitch */}
          <h1 style={{
            fontSize: "clamp(52px,9vw,80px)", fontWeight: "800",
            lineHeight: "1", letterSpacing: "-0.05em", marginBottom: "14px",
            fontFamily: "'IBM Plex Mono',monospace", color: "#f0f4f8",
          ...
          <div style={{ display: "flex", justifyContent: "center", gap: "40px", marginTop: "28px", paddingTop: "24px", borderTop: "1px solid rgba(255,255,255,0.05)" }}>
            {[["$1T+", "Lost to scams/year"], ["3.2B", "Scam msgs daily"], ["Free", "Always"]].map(([val, label], i) => (
              <div key={label} style={{ textAlign: "center", animation: `countIn 0.6s ${0.3 + i * 0.15}s both` }}>
                <div style={{ fontSize: "18px", fontWeight: "700", color: "#ffb800", background: "linear-gradient(90deg,#ffb800,#ffd000,#ffb800)", backgroundSize: "200% auto", WebkitBackgroundClip: "text", WebkitTextFillColor: "transparent", animation: "shimmer 3s linear infinite" }}>{val}</div>
            ...
            {EXAMPLES.map((ex, i) => (
              <button key={ex.label} onClick={() => { setText(ex.text); setResult(null); }}
                style={{
                  ...
                  color: "rgba(200,212,224,0.65)", padding: "7px 13px", borderRadius: "4px",
                  fontSize: "11px", cursor: "pointer", fontFamily: "inherit",
                  transition: "all 0.25s", animation: `fadeSlideUp 0.5s ${0.1 * i}s both`,
                }}
        ...
        <div style={{ position: "relative", marginBottom: "14px" }}>
          <textarea value={input} onChange={e => { setText(e.target.value); setResult(null); }}
            ...
            style={{
              ...
              padding: "20px", color: "#f0f4f8", fontSize: "14px", lineHeight: "1.75",
          ...
          {input && (
            <button onClick={() => { setText(""); setResult(null); }}
              style={{ position: "absolute", top: "12px", right: "12px", background: "none", border: "none", color: "rgba(200,212,224,0.25)", cursor: "pointer", fontSize: "18px", lineHeight: 1, padding: "4px", transition: "color 0.2s" }}
              onMouseEnter={e => e.currentTarget.style.color = "#ff3b3b"}
              onMouseLeave={e => e.currentTarget.style.color = "rgba(200,212,224,0.25)"}
            >✕</button>
          )}
        ...
        <button onClick={analyze} disabled={loading || !input.trim()}
          style={{
            ...
            background: loading ? "rgba(255,184,0,0.12)" : (!input.trim() ? "rgba(255,184,0,0.08)" : "linear-gradient(135deg,#ffb800,#ffd000)"),
            border: "1px solid", borderColor: !input.trim() ? "rgba(255,184,0,0.15)" : "rgba(255,184,0,0.6)",
            borderRadius: "8px",
            color: loading || !input.trim() ? "rgba(255,184,0,0.4)" : "#080c12",
            ...
            boxShadow: input.trim() && !loading ? "0 0 30px rgba(255,184,0,0.2)" : "none",
          }}
        ...
        {error && (
          <div style={{ marginTop: "16px", padding: "14px", background: "rgba(255,59,59,0.08)", border: "1px solid rgba(255,59,59,0.2)", borderRadius: "8px", color: "#ff6b6b", fontSize: "13px", animation: "fadeSlideUp 0.3s both" }}>
            ⚠ {error}
          </div>
        )}
        ...
        {result && cfg && (
          <div style={{
            ...
            borderRadius: "14px", background: cfg.bg, boxShadow: cfg.glow,
            overflow: "hidden", animation: "fadeSlideUp 0.6s cubic-bezier(0.16,1,0.3,1)",
          }}>

            {/* Radar + Verdict header */}
            <div style={{ padding: "32px 28px 24px", borderBottom: `1px solid ${cfg.border}`, textAlign: "center" }}>
              ...
              <div style={{ maxWidth: "300px", margin: "0 auto" }}>
                <div style={{ display: "flex", justifyContent: "space-between", fontSize: "11px", color: "rgba(200,212,224,0.4)", marginBottom: "8px" }}>
                  ...
                  <Counter target={result.riskScore} color={cfg.color} />
                </div>
                <div style={{ height: "8px", background: "rgba(255,255,255,0.06)", borderRadius: "4px", overflow: "hidden" }}>
                  <div style={{
                    height: "100%", borderRadius: "4px",
                    width: `${result.riskScore}%`,
                    background: result.riskScore > 70 ? "linear-gradient(90deg,#ff8c00,#ff3b3b)" : result.riskScore > 40 ? "linear-gradient(90deg,#ffd000,#ffb800)" : "linear-gradient(90deg,#00b368,#00d97e)",
            ...
            {/* Red Flags */}
            <div style={{ padding: "22px 28px", borderBottom: `1px solid ${cfg.border}` }}>
              <div style={{ fontSize: "10px", color: "rgba(200,212,224,0.35)", letterSpacing: "0.1em", textTransform: "uppercase", marginBottom: "14px" }}>
              ...
              <div style={{ display: "flex", flexDirection: "column", gap: "10px" }}>
                {result.whatToDo.map((action, i) => (
                  <div key={i} style={{
                    ...
                    animation: `flagIn 0.4s ${0.2 + i * 0.1}s both`,
                    transition: "all 0.2s",
                  }}
